...
wrapper.authenticate = function(options) {
  ...
  console.log('saml-hapi authenticate options: ' + JSON.stringify(options));
  ...
  options.verifyFunc = options.verifyFunc || function(profile, done) {
    console.log("Inside the default verifyFunc with profile: " + JSON.stringify(profile));
  ...
  return function(request, reply) {
    function validateCallback(err, profile, loggedOut) {
      ...
      console.log("in validate callback, with profile: " + JSON.stringify(profile));
      ...
      var verified = function (err, user, info) {
        console.log("in the verified function with user: " + JSON.stringify(user) + ", and info: " + JSON.stringify(info));
...
wrapper.logout = function(settings, logoutOptions, callback) {

  var saml = new Saml.SAML(settings);
  
  saml.getLogoutUrl(logoutOptions, function(err, url) {
    
    if (err !== null) { callback(err, url); }
